import streamlit as st import json import os from fpdf import FPDF from datetime import datetime import gspread from google.oauth2.service_account import Credentials from pydrive.auth import GoogleAuth from pydrive.drive import GoogleDrive

-------- CONFIGS --------

ARQ_PRODUTOS = "produtos.json" PASTA_PDFS = "orcamentos" SHEET_NAME = "Nome da Planilha" JSON_CRED_PATH = "Planilhas.json" DRIVE_FOLDER_ID = "0B8YxMAd2J3kFckV4VjVhV1Y1NE0"  # Substitua com o ID da pasta no seu Drive
...
def enviar_para_drive(caminho_arquivo): gauth = GoogleAuth() gauth.LoadCredentialsFile(JSON_CRED_PATH) if gauth.credentials is None: gauth.LocalWebserverAuth() else: gauth.Authorize() drive = GoogleDrive(gauth)

arquivo = drive.CreateFile({'title': os.path.basename(caminho_arquivo), 'parents': [{'id': DRIVE_FOLDER_ID}]})
...
def main(): st.title("AQUI-DECK App")
        ...
        if st.button("Gerar PDF"):
            if nome_cliente.strip() and contato.strip():
                ...
                try:
                    ...
                    st.success("PDF salvo e enviado ao Google Drive com sucesso!")
