...
JSON_CRED_PATH = os.getenv("JSON_CRED_PATH", "Planilhas.json")  # Usar variável de ambiente
CRED_FILE = os.getenv("CRED_FILE", "mycreds.txt")
    ...
    try:
        if not os.path.exists(ARQ_PRODUTOS):
            with open(ARQ_PRODUTOS, "w") as f:
                json.dump({"Fixos": [], "Produtos": [], "Alteracao": []}, f)
        with open(ARQ_PRODUTOS, "r") as f:
            return json.load(f)
    except Exception as e:
        st.error(f"Erro ao carregar dados: {e}")
        return {"Fixos": [], "Produtos": [], "Alteracao": []}
...
def main():
    ...
    modo = st.sidebar.selectbox("Modo", ["Cadastro", "Gerenciar Produtos", "Orçamento"])
